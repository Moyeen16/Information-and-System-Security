...
void pre_key_mixing(int* inputText, int* key, int block_size)
{
    for(int i=0; i<block_size; i++)
        inputText[i] = inputText[i] ^ key[i];
}
...
void compute(int* permutation_key, int*substitution_func, char ch)
{
    ...
    printf("Enter the Initialisation Vector (%d bits)\n", block_size);
    for(int i=0; i<block_size; i++)
        scanf("%1d", &outputText[i]);
    fflush(stdin);
    fflush(stdin);
    ...
    for(int bl=1; bl<=number_of_blocks; bl++)
    {
        ...
        for(int i=0; i<block_size; i++)
        {
            ...
            if(ch == 'd' && bl==1)
                inputText_copy[i] = outputText[i];
            else
                inputText_copy[i] = full_inputText[8*(bl-2)+i];
            interText1[i] = 0;
            interText2[i] = 0;
            interText3[i] = 0;
        }
        ...
        if(ch == 'e')
        {
            pre_key_mixing(inputText, outputText, block_size);
        ...
        else if(ch == 'd')
        {
            ...
            pre_key_mixing(outputText, inputText_copy, block_size);
        }
        //Decryption Ends
