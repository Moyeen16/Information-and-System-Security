...
void decrypt()
{
    ...
    printf("(Miltiplicative Inverse for Key 1 : %d)\n", mulInv);
    printf("(Addivive Inverse for Key 2 : %d)\n", addInv);
...
void encrypt()
{
    ...
    printf("Enter the value of Key 2.\n");
    scanf("%d", &key2);
    int inv = multiplicativeInverse(key1, mod);
    if(inv == 0)
        return;
    ...
    for(int i=0; i<characters-1; i++)
    {
        ...
        cipherText[i] = (char)(((plainText[i] - 97) * key1 + key2)% mod + 65);
    }
...
int main()
{
    ...
    while(ch != 3)
    {
        printf("\n************* Demonstration of Affine Cipher *************\n");
        printf("1. Encrypt\n");
        printf("2. Decrypt\n");
        printf("3. Exit\n");
        scanf("%d", &ch);
